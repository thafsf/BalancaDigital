...
float FATOR_DE_CALIBRACAO = 449500; 
...
<head>
  ...
  <style>
    ...
    /* Estilos para o mostrador analógico */
    .scale {
      position: relative;
      width: 300px;
      height: 150px;
...
<body>
  ...
  function updateWeightDisplay(weight) {
    ...
    // Atualiza o ponteiro
    const angle = (weight / MAX_WEIGHT) * 180; // Mapeia o peso para um ângulo de 0 a 180 graus
    pointerElement.style.transform = `translateX(-50%) rotate(-${angle}deg)`;
  }
...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    if (scale.is_ready() && ws.count() > 0) {
      float peso = scale.get_units(100);
